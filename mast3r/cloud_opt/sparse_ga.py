...
def decoder(model, feats, poss, shapes, return_attention=False):
    decoder_input = tuple(x for pair in zip(feats, poss) for x in pair)
    model_output = model._decoder(*decoder_input, return_attention=return_attention)
    if return_attention:
        model_output, attn = model_output
    decs = tuple(model_output)
    with torch.amp.autocast("cuda", enabled=False):
        ress = tuple(
            model._downstream_head(min(i + 1, 2), [tok.float() for tok in dec], shape)
            for i, (dec, shape) in enumerate(zip(decs, shapes))
        )
    if return_attention:
        ress[0]["attn"] = attn
    return ress
...
def symmetric_inference(model, *imgs, device, return_attention=False):
    shapes = tuple(
        torch.from_numpy(img["true_shape"]).to(device, non_blocking=True)
        for img in imgs
    )
    imgs = tuple(img["img"].to(device, non_blocking=True) for img in imgs)
    num_images = len(imgs)

    # compute encoder only once
    feats_poss = model._encode_image_pairs(*imgs, *shapes)
    feats = feats_poss[:num_images]
    poss = feats_poss[num_images:]

    # decoder 1-2
    # res11, res21 = decoder(feats[::1], poss[::1], shapes[::1])
    # # decoder 2-1
    # res22, res12 = decoder(feat2, feat1, pos2, pos1, shape2, shape1)

    # feats, poss, shapes = feats[::-1], poss[::-1], shapes[::-1]
    ...
    return tuple(x for st in resss for x in st)
